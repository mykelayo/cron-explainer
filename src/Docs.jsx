...
function Playground({ C }) {
  ...
  async function run() {
    ...
    try {
      // Use window.location.origin so staging/preview deploys
      // call their own API instead of hardcoded production. Same-origin also
      // satisfies CSP connect-src 'self' on all environments.
      const apiBase = typeof window !== "undefined" ? window.location.origin : SITE_URL;
        ...
        <main style={{ flex: 1, padding: "clamp(24px,4vw,44px) clamp(16px,4vw,40px) 80px", minWidth: 0 }}>
          ...
          <section id="overview" style={{ maxWidth: "700px", marginBottom: "56px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: "1px", border: `1px solid ${C.border}`, borderRadius: "4px", overflow: "hidden" }}>
              {[["BASE URL", SITE_URL], ["AUTH", "None required"], ["RATE LIMIT", "60 req / min / IP"], ["FORMAT", "JSON"]].map(([l, v]) => (
  ...
  "nextRuns": [
    ...
    "2026-02-26T09:00:00.000Z"
          ...
          <section id="examples" style={{ maxWidth: "700px", marginBottom: "56px" }}>
            ...
            {/* overflow was "hidden" — changed to "auto" so long lines scroll */}
